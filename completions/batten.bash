_batten() {
    ...
    case "${cmd}" in
        ...
        batten__subcmd__payload__subcmd__field)
            ...
            case "${prev}" in
                --harness)
                    ...
                    ;;
                --name)
                    COMPREPLY=($(compgen -W "hook-event-name session-id tool-name command cwd stop-hook-active last-assistant-message transcript-path prompt run-in-background" -- "${cur}"))
                    return 0
                    ;;
                --strictness)
                    COMPREPLY=($(compgen -W "permissive standard strict" -- "${cur}"))
        ...
        batten__subcmd__policy)
            opts="-q -v -y -h --strictness --fail-on-warning --config-from --silent --quiet --verbose --debug --trace --log-level --no-color --no-input --yes --help budget test help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
