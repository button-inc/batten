...
complete -c batten -n "__fish_batten_using_subcommand config; and not __fish_seen_subcommand_from show help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
...
complete -c batten -n "__fish_batten_using_subcommand generate; and not __fish_seen_subcommand_from completions help" -l strictness -d 'Raise how strictly gates apply (an override may only tighten policy)' -r -f -a "permissive\t'Advisory: findings are reported without failing the run'
standard\t'The default: a finding is a violation'
strict\t'Everything `Standard` fails on, plus anything advisory'"
complete -c batten -n "__fish_batten_using_subcommand generate; and not __fish_seen_subcommand_from completions help" -l fail-on-warning -d 'Promote a warn-severity finding to a violation (an override may only turn this on)'
complete -c batten -n "__fish_batten_using_subcommand generate; and not __fish_seen_subcommand_from completions help" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c batten -n "__fish_batten_using_subcommand generate; and not __fish_seen_subcommand_from completions help" -f -a "completions" -d 'Emit the shell completion script for one shell'
complete -c batten -n "__fish_batten_using_subcommand generate; and not __fish_seen_subcommand_from completions help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
...
complete -c batten -n "__fish_batten_using_subcommand help; and not __fish_seen_subcommand_from check enforce config spec generate hook receipt help" -f -a "check" -d 'Run the applicable read-only gates against the repository'
complete -c batten -n "__fish_batten_using_subcommand help; and not __fish_seen_subcommand_from check enforce config spec generate hook receipt help" -f -a "enforce" -d 'Run every configured rule, including kinds that execute a configured command'
complete -c batten -n "__fish_batten_using_subcommand help; and not __fish_seen_subcommand_from check enforce config spec generate hook receipt help" -f -a "config" -d 'Inspect configuration'
complete -c batten -n "__fish_batten_using_subcommand help; and not __fish_seen_subcommand_from check enforce config spec generate hook receipt help" -f -a "spec" -d 'Print the tool\'s own command spec'
complete -c batten -n "__fish_batten_using_subcommand help; and not __fish_seen_subcommand_from check enforce config spec generate hook receipt help" -f -a "generate" -d 'Emit artifacts derived from the command spec, on stdout'
complete -c batten -n "__fish_batten_using_subcommand help; and not __fish_seen_subcommand_from check enforce config spec generate hook receipt help" -f -a "hook" -d 'Adjudicate a mediated tool call read from stdin (a deny is exit 2, the one contract)'
complete -c batten -n "__fish_batten_using_subcommand help; and not __fish_seen_subcommand_from check enforce config spec generate hook receipt help" -f -a "receipt" -d 'Verification receipts: SHA-keyed claims a named check passed, invalidated by git facts'
complete -c batten -n "__fish_batten_using_subcommand help; and not __fish_seen_subcommand_from check enforce config spec generate hook receipt help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
