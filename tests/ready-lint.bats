...
# --- resolving the payload instead of being handed it (CLOUD-1121) ------------
#
# The arm that makes a lint cost nothing in context: `--issue <KEY>` reads the
# payload out of the capture store, where the engine wrote it when the read
# happened, so nothing has to be piped in and nothing has to have entered an
# agent's context for this gate to decide.
#
# Both cases below run with stdin CLOSED, which is the only way to tell this arm
# from the old one: an implementation that quietly kept reading stdin passes
# every test that leaves a payload on it.

# A git repository with an isolated capture store, and `batten hook` driven at
# the post-tool event so the ENGINE writes the capture. Never a hand-written blob
# and log line: a fixture built to the reader's shape cannot prove the writer
# emits it, which is the defect this same change repaired in `claim-check`.
seed_capture() { # seed_capture <tool> <json>
	export XDG_DATA_HOME="$BATS_TEST_TMPDIR/store-$BATS_TEST_NUMBER"
	export HOME="$BATS_TEST_TMPDIR/home-$BATS_TEST_NUMBER"
	mkdir -p "$XDG_DATA_HOME" "$HOME"
	STORE_REPO="$BATS_TEST_TMPDIR/repo-$BATS_TEST_NUMBER"
	mkdir -p "$STORE_REPO"
	git -C "$STORE_REPO" init -q
	# The MCP content-block shape, which is what a host hands over and the only
	# thing `decode_response` reads for a tool call. A bare object is recorded as
	# an unreadable shape with no digest, and the resolver then never finds it.
	jq -nc --arg t "$1" --arg r "$2" \
		'{hook_event_name:"PostToolUse", session_id:"ready-lint-suite", tool_name:$t, tool_input:{}, tool_response:[{type:"text", text:$r}]}' |
		(cd "$STORE_REPO" && batten hook --harness claude-code >/dev/null 2>&1)
}

@test "--issue against an empty capture store is could-not-look, never a verdict" {
	# The discriminator for `resolve-falls-through-to-stdin`, and the assertion
	# that discriminates is the MESSAGE rather than the status: under the
	# mutation the fall-through reads an empty stdin, fails the `.description`
	# check and exits 2 as well. Same code, different question answered — which
	# is exactly the confusion this arm exists to prevent, so the case asserts
	# WHICH refusal fired.
	export XDG_DATA_HOME="$BATS_TEST_TMPDIR/empty-store"
	export HOME="$BATS_TEST_TMPDIR/empty-home"
	mkdir -p "$XDG_DATA_HOME" "$HOME"
	local repo="$BATS_TEST_TMPDIR/empty-repo"
	mkdir -p "$repo"
	git -C "$repo" init -q
	run bash -c "cd '$repo' && '$LINT' --issue CLOUD-999 </dev/null"
	[ "$status" -eq 2 ]
	[[ "$output" == *"no stored payload for CLOUD-999"* ]]
	# The mutation's tell: having fallen through, it judges an empty stdin and
	# reports the input problem instead of the lookup one.
	[[ "$output" != *"stdin is not a get_issue payload"* ]]
	# And it must never reach a verdict ABOUT THE ISSUE from a store that holds
	# nothing — an absent payload reading as an unrefined row is the false green.
	[[ "$output" != *"no-ready-block"* ]]
}

@test "--issue reaches the same verdict as the piped payload, with stdin closed" {
	# The row's acceptance criterion, stated as a comparison rather than as an
	# absolute: the same body, once through the pipe and once through the store,
	# must produce the same answer. A case asserting only "exit 0" would pass on
	# an implementation that resolved the wrong payload.
	payload "$(block)"
	local piped_status piped_output
	piped_output=$("$LINT" <"$PAYLOAD" 2>&1) && piped_status=0 || piped_status=$?

	seed_capture mcp__tracker__get_issue "$(cat "$PAYLOAD")"
	run bash -c "cd '$STORE_REPO' && '$LINT' --issue CLOUD-999 </dev/null"
	[ "$status" -eq "$piped_status" ]
	[[ "$output" == "$piped_output" ]]
}
