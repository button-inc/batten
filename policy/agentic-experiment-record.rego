...
# THE PARAMETER IS `doc` AND MUST NOT BE `object`, which is a second measured
# correction on the same three lines. Naming it `object` shadows the `object.*`
# builtin namespace inside this body — regorus resolves the builtins through the
# shadowing parameter and ran all 22 cases below green, and `opa check -s` refuses
# it with two `undefined function` errors. A module only one of the two engines
# accepts is a gate whose behaviour depends on which binary reads it, which is why
# `policy-modules-type-check` exists and why it is the gate that found this.
replacing(doc, key, value) := object.union(object.remove(doc, {key}), {key: value})
