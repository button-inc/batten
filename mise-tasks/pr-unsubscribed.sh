...
# Anchored on BOTH tabs, because the line lives inside `give_up` inside the verb
# block — a one-tab pattern matched nothing and `mutant` called it `inert`, which
# is the right verdict and a quieter failure than an unappliable one. Anchored on
