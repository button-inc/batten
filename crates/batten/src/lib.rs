...
fn run_hook(
    ...
    // carrying a `batten.toml` that will not parse: a `Read` envelope denied at
    // exit `2`, its reason carrying the parse position the agent was thereby
    // denied the means to look at. A container in that state cannot look, cannot
    // edit, cannot diagnose and cannot report, and each declared escape is
    // unreachable from inside it — the env hatch above cannot be spelled on the
    // Bash surface that advertises it (CLOUD-1605), and an override over a
    // config that will not load cannot be minted (CLOUD-1579).
    //
...
/// the second denies. This adds a third case belonging with neither: the
/// authority is unreadable AND the call carries nothing any rule could decide.
///
/// # THE OPERATION IS ASKED, NOT INFERRED FROM AN ABSENCE
///
/// A first attempt read `command.is_empty() && writes.is_none()` and called that
/// "cannot mutate". **It is not.** [`hook::Harness::operation_of`] maps an MCP
/// call to [`hook::Operation::Mcp`], a subagent spawn to `Subagent` and an
/// unrecognised tool to `Other` — and every one of those carries an empty
/// command and no write, exactly as [`is_adjudicable`]'s own doc says. Measured
/// on a scratch fixture before the correction: `mcp__serena__write_memory`,
/// whose target is a **protected** path, was allowed at exit `0` over an
/// unreadable config, and so was a `Task` spawn. That is the class CLOUD-1677
/// measured, reintroduced by the change meant to put a floor under it.
///
/// So the predicate NAMES the operations it admits and refuses everything else,
/// which is the direction [`hook::Operation::Other`]'s own doc demands: it is
/// *"something specific was seen and not recognized"* — could-not-look — and
/// reading a could-not-look as "harmless" is the inference this engine exists to
/// refuse.
///
/// **Two exemptions, each an absence rather than a permission.**
///
/// * **A classified READ.** [`hook::Operation::Read`] with nothing written. Such
///   an envelope reaches [`is_adjudicable`] only through its
///   `PreTool`-with-a-tool-name clause, whose own doc says it exists so
///   *tool-keyed rows* can match. Under an unreadable config the row count is
///   zero, so that clause is answering a question with nothing on either side of
///   it. The cost of refusing anyway is total: the `Read` that would show the
///   agent the parse error is the very call denied.
/// * **A write to the config authority itself.** The one mutation whose whole
///   effect is on the file that is faulting, and therefore the only mutation
///   that can END the degraded state. Refusing it makes the fault permanent,
///   which is CLOUD-1579's lockout reached by a second route.
///
/// # What is NOT exempt
///
...
///
/// # Two bounds on the read half, stated rather than discovered
///
/// `Grep` and `Glob` are `Other` on every host, so they refuse here too. And
/// `operation_of`'s `GeminiCli | CopilotCli` arm returns `Other` for every
/// non-writer tool, so **no call on those two hosts ever yields `Read`** and
/// this exemption is inert there — the repair-write half still works, because
/// `write_tools` is consulted before the per-host arm. Both are inherited from
/// CLOUD-209's unsurveyed hosts rather than introduced here, and CLOUD-1855
/// carries the fix.
///
...
fn recoverable_without_rules(envelope: &hook::Envelope) -> bool {
    ...
    // whether or not such a guard runs — and no envelope can carry a command
    // while classifying as `Read` or `Write`, because those tools' inputs have
    // no `command` key. A guard here would be unreachable for the deny
    // direction, so its declared mutation could never die: a dead gate in a
    // module the census enforces, which is what CLOUD-418 refuses.
...
///
/// One reader for [`fire_actions`] and [`dispatch_handlers`], which carried the
/// same four lines each. Both want the REPOSITORY's authority rather than the
/// cwd's (CLOUD-824), and both want an absent table to mean "nothing declared".
///
...
/// from? (CLOUD-1842)
///
/// Whole-name equality after the one normalization that preserves identity: `\`
/// folded to `/`, because a host spelling the same file either way means the
/// same file. **No `trim()`** — `" batten.toml"` and `"batten.toml "` are
/// different files on a POSIX filesystem, and trimming would hand the exemption
/// to them. The first draft borrowed the trim from [`crate::bypass`], where it
/// is right because that code compares two CALLS to each other and surrounding
/// whitespace is transcription noise; here the comparison is an identity check
/// against a constant, where whitespace is part of the name.
///
/// No glob, no prefix test, no path resolution: `crates/batten.toml` ends with
/// the authority's name and is a different file.
