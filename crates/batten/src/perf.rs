...
pub struct RenderRecord {
    ...
    /// The declared class token. With the declared ceiling it identifies the
    /// baseline: those are the two inputs the rendering reads.
    pub class: String,
...
fn refusal_render_preamble() -> String {
    ...
    out.push_str(
        "Generated by `mise run refusal-render-bench`. A REPORT, never a gate: no task in \
         `verify` or the hk gate reads it, and it exits 0 whatever the numbers say.\n\n",
    );
    out.push_str(
        "**NO CRATE VERSION AND NO COMMIT SHA**, and the second absence is the first one's \
         argument applied twice. CLOUD-1606 asked for the version as the baseline's identity; a \
         version in the body is not byte-stable under RELEASE, because release-plz bumps it in a \
         commit that changes no rendering — so the tier's drift check would redden every release \
         and the report would have to be regenerated to say a number nobody measured against. \
         What identifies this baseline is what the rendering actually reads: the declared classes \
         below and the ceiling named next.\n\n",
    );
    out.push_str(
