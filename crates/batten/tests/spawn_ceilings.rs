...
//! suite-qualified.
//!
// carried: "fanout-guard.bats::an ordinary single-target spawn is allowed" crates/batten/tests/spawn_ceilings.rs
// carried: "fanout-guard.bats::a manifest over the cap is refused, naming the cap and the count" crates/batten/tests/spawn_ceilings.rs
// carried: "fanout-guard.bats::a mem: reference counts as an artifact, resolved against the tree" crates/batten/tests/spawn_ceilings.rs
// carried: "fanout-guard.bats::a path-shaped token naming nothing tracked does not count" crates/batten/tests/spawn_ceilings.rs
// carried: "fanout-guard.bats::an oversize prompt is refused against the token budget" crates/batten/tests/spawn_ceilings.rs
// carried: "fanout-guard.bats::a tool that is not a spawn is never judged" crates/batten/tests/spawn_ceilings.rs
// carried: "fanout-guard.bats::an absent prompt fails open" crates/batten/tests/spawn_ceilings.rs
// carried: "fanout-guard.bats::the refusal is a pointer — it carries no prompt bytes" crates/batten/tests/spawn_ceilings.rs
//!
//! SUBSUMED — the plumbing became the engine's, which is what a migration should
//! produce.
//!
// subsumed: "fanout-guard.bats::unparseable stdin neither refuses nor errors" crates/batten/tests/cli.rs
...
//! dropped.
//!
// changed: "fanout-guard.bats::the caps are configurable in both directions" crates/batten/tests/spawn_ceilings.rs BATTEN_FANOUT_READING_CAP and BATTEN_FANOUT_PROMPT_BUDGET are gone: each cap is `max` on its own row, configured where every other property of the row is. Per-call override is deliberately not carried — an agent that can raise the ceiling at the call being gated is not gated — and the two directions are carried as the at-cap and over-cap cases below
// changed: "fanout-guard.bats::the bypass is honoured" crates/batten/tests/guardrail_bypass.rs BATTEN_FANOUT_GUARD_BYPASS is gone; a mediated deny takes the engine's own hatch, the consolidation rows 1-3 record
//!
