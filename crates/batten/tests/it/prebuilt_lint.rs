...
//
// carried: "this repository is clean today — the rule is green on the tree it governs" batten.toml the two rows now run over this checkout from crates/batten/tests/it/prebuilt_lint.rs, one rule set at a time rather than the whole config in a fixture
#[test]
...
// carried: "a prebuilt install-action step is not a violation" crates/batten/tests/it/prebuilt_lint.rs
#[test]
...
//
// carried: tests/prebuilt-lint.bats crates/batten/src/rules.rs kind:mechanism crates/batten/tests/it/prebuilt_lint.rs
//
// The four waiver cases are NOT here: they were about the waiver SURFACE, generic
// over which `forbid` row it suppresses, and `crates/batten/tests/it/waivers.rs`
