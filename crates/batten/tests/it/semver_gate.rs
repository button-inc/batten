...
//!
// carried: mise-tasks/semver.sh crates/batten/src/semver.rs kind:verb crates/batten/tests/it/semver_gate.rs
// carried: tests/semver.bats crates/batten/src/semver.rs kind:verb crates/batten/tests/it/semver_gate.rs
//!
//! # RETIREMENT LEDGER — `tests/semver.bats`, 12 cases
//!
//! CARRIED — the property survives, proved here against the binary.
//!
// carried: "a patch-compatible delta passes, and names the claim it verified" crates/batten/tests/it/semver_gate.rs
// carried: "THE VACUOUS RUN: a report that graded 0 checks is exit 2, never a pass" crates/batten/tests/it/semver_gate.rs
// carried: "an undeclared break fails, and names the lint rather than the payload" crates/batten/tests/it/semver_gate.rs
// carried: "a break declared with a bang passes, and names the declaring commit" crates/batten/tests/it/semver_gate.rs
// carried: "a break declared with a BREAKING CHANGE footer passes too" crates/batten/tests/it/semver_gate.rs
// carried: "A DECLARATION ON THE BASELINE DOES NOT COUNT — only this branch's commits" crates/batten/tests/it/semver_gate.rs
// carried: "an exit code that is neither verdict is exit 2 — a broken run is not a pass" crates/batten/tests/it/semver_gate.rs
// carried: "output is a pointer — lint ids and a short sha, never the rustdoc it read" crates/batten/tests/it/semver_gate.rs
//!
