...
//
// carried: "the first call is the session's start: silent, and it writes a snapshot" crates/batten/tests/it/contract_drift.rs
// carried: "an unchanged surface produces no output" crates/batten/src/contract.rs kind:mechanism
// carried: "THE GAP: a modified AGENTS.md is reported, naming the file" crates/batten/tests/it/contract_drift.rs
// carried: "it names the event it was called on, so one body serves both wirings" crates/batten/tests/it/contract_drift.rs
// carried: "ONCE PER CHANGE-SET: the very next call is silent" crates/batten/tests/it/contract_drift.rs
// carried: "a SECOND change-set is reported again — quiet is not permanent" crates/batten/tests/it/contract_drift.rs
// carried: "a newly tracked contract file is drift" crates/batten/tests/it/contract_drift.rs
// carried: "a contract file that stopped being tracked is drift too" crates/batten/tests/it/contract_drift.rs
// carried: "a file outside the surface does not fire it" crates/batten/tests/it/contract_drift.rs
// carried: "each session gets its own snapshot, so a session that started AFTER the change is not nudged" crates/batten/tests/it/contract_drift.rs
...
// carried: "the reminder carries no byte of the changed file's content" crates/batten/tests/it/contract_drift.rs
// carried: "when settings.json moved it says a new hook may not be loaded in this session" crates/batten/tests/it/contract_drift.rs
// carried: "outside a checkout there is no surface to judge" crates/batten/tests/it/contract_drift.rs
//
...
// subsumed: "unparseable input fails open" crates/batten/tests/it/cli.rs
// subsumed: "empty input fails open" crates/batten/tests/it/cli.rs
// subsumed: "it emits a count as well as the paths" crates/batten/src/contract.rs kind:mechanism
// subsumed: "the emitted document is the hook shape, and it parses" crates/batten/tests/it/advisory_drain.rs
//
...
//
// changed: "an untracked file under mise-tasks is not contract" crates/batten/tests/it/contract_drift.rs the surface is globs now, so a newly added gate IS the drift — `[epoch] tracked`'s literal paths structurally cannot see a file that postdates the list
// changed: "the bypass is honoured" crates/batten/tests/it/contract_drift.rs BATTEN_CONTRACT_DRIFT_BYPASS is gone and the engine's hatch deliberately does not reach an advisory: it carries no verdict and refuses nothing, so a switch over it would only suppress news
