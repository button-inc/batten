...
#MUTANT rebuild-the-lease-time-inline|s@        time: crate::utc_at(seconds),@        time: gix::date::Time { seconds, offset: 3600 },@|a_lease_object_is_stamped_in_utc
*/
...
///
/// **What it does not assert:** that the offset renders as `+0000` in the
/// serialised bytes. That is `gix`'s formatter's business, and a case pinning it
/// would be asserting against the library rather than against this crate.
...
fn a_lease_object_is_stamped_in_utc() {
    ...
    // THE ASSERTION IS ON THE SERIALISED BYTES, not on a decoded signature, and
    // that is the stronger reading rather than the lazier one: the id is a hash of
    // exactly these bytes, so the header is where the property lives. A decode
    // would also accept a rendering that parsed back to UTC by some other route.
    assert_eq!(
        stamp(&raw, "committer "),
