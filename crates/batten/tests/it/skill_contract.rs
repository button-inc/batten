...
//
// carried: mise-tasks/skill-check.sh crates/batten/src/surface.rs kind:mechanism crates/batten/tests/it/skill_contract.rs
// carried: tests/skill-check.bats mise-tasks/skill-check.sh crates/batten/src/surface.rs kind:mechanism crates/batten/tests/it/skill_contract.rs
//
...
//
// carried: "skill-check.bats::a skill inside budget, naming only declared verbs, exits 0" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::a verb the binary does not declare is reported with a file:line pointer" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::a subcommand the binary does not declare is caught, not just a bare verb" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::a positional argument does not read as an undeclared subcommand" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::flags do not read as subcommands" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::a console block is judged as well as an inline span" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::prose naming the product is not read as a verb" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::a skill over the line budget is refused, and the count is named" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::the budget is a boundary, not a suggestion" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::an exit meaning that drifts from the binary's rendering is caught" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::a code the skill never names is caught even when its meaning is present" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::a vendor path that is a copy rather than a symlink is refused" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::a symlink pointing at some other file is refused" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::a missing skill is exit 2 — could not look, not a clean tree" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::the repo as it stands passes" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::a second skill with no vendor symlink is a violation" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::a second skill is discovered while UNTRACKED — presence is the predicate" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::a well-formed second skill leaves the run clean" crates/batten/tests/it/skill_contract.rs
// carried: "skill-check.bats::a second skill over budget is reported against its own path" crates/batten/tests/it/skill_contract.rs
//
// changed: "skill-check.bats::an unreadable spec is exit 2, never a pass over an unjudged vocabulary" crates/batten/tests/it/skill_contract.rs the suite pointed `BATTEN_BIN` at a nonexistent path, and this tier has no such indirection: `common::batten()` resolves `CARGO_BIN_EXE_batten`, so a binary that is not there is a harness that did not build rather than a verdict. What that case protected — that an unjudged vocabulary never reads as a clean one — survives as `an_empty_vocabulary_is_never_read_as_a_clean_skill`, which asserts the refusal from the reading's side instead of from the launcher's
// changed: "skill-check.bats::the gate is wired: hk.pkl declares a step that runs this task" crates/batten/tests/it/skill_contract.rs the step now runs the successor task rather than `mise run skill-check`, so the literal it asserted is gone. The property is the same one and is still gated: `the_skill_contract_is_wired_into_the_hk_gate` asserts hk.pkl declares a `skill-check` step whose check names the task that runs THIS file
...
fn the_skill_contract_is_wired_into_the_hk_gate() {
    ...
    assert!(
        ...
        "the skill-check step must run the task that drives \
         crates/batten/tests/it/skill_contract.rs, and runs `{check}`"
    );
}
