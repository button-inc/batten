...
//
// carried: mise-tasks/reference-check.sh crates/batten/src/render.rs kind:mechanism crates/batten/tests/it/reference_coverage.rs
// carried: tests/reference-check.bats mise-tasks/reference-check.sh crates/batten/src/render.rs kind:mechanism crates/batten/tests/it/reference_coverage.rs
//
...
//
// carried: "reference-check.bats::a reference naming every declared flag passes" crates/batten/tests/it/reference_coverage.rs
// carried: "reference-check.bats::a flag the reference omits is reported with its name" crates/batten/tests/it/reference_coverage.rs
// carried: "reference-check.bats::a flag the reference invents is reported with its name" crates/batten/tests/it/reference_coverage.rs
// carried: "reference-check.bats::both directions are reported in one run, not just the first" crates/batten/tests/it/reference_coverage.rs
// carried: "reference-check.bats::output is pointer-only — no line of the reference echoed" crates/batten/tests/it/reference_coverage.rs
// carried: "reference-check.bats::a reference naming no flags at all is could-not-look, never a pass" crates/batten/tests/it/reference_coverage.rs
// carried: "reference-check.bats::the gate leaves no reference behind in the tree it judges" crates/batten/tests/it/reference_coverage.rs
// carried: "reference-check.bats::this repo's reference covers its surface — the gate on the real tree" crates/batten/tests/it/reference_coverage.rs
//
// changed: "reference-check.bats::a renderer that fails is could-not-look, never a pass" crates/batten/tests/it/reference_coverage.rs the suite stubbed a sibling program that exited 1, and there is no sibling to stub: the renderer is the binary under test. The property that survives is the one the stub stood in for — a render that did not produce a reference must not read as a covered one — asserted in `a_render_that_did_not_happen_is_never_read_as_coverage`, which drives the real binary to a non-zero exit and shows the empty reading is refused rather than passed
// changed: "reference-check.bats::an absent renderer is could-not-look, never a pass" crates/batten/tests/it/reference_coverage.rs same cause, one case further on: an ABSENT renderer is unreachable once the renderer is the binary, because a missing binary is a test harness that did not build rather than a verdict this tier can reach. The reading it protected — that an unusable render is not coverage — is the same one `a_render_that_did_not_happen_is_never_read_as_coverage` carries, so this arm records the collapse rather than claiming two cases survived
