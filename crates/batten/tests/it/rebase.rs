...
//!
//! The section above says why a `gitwrite` mutation is DECLARED here. What it
...
//! behind the `engine-` prefix, or a preset directory. No gate name resolves to a
//! test file.
//!
...
//! contradiction belongs to those two gates and is measured on CLOUD-1369.
//!
...
/// signature rather than the tree.
fn commit_by(
    repo: &gix::Repository,
...
/// so the case separates *pinning the committer* from *flattening the object*
/// without depending on the runner's zone.
///
...
/// token is the whole of what this asks about. Read off the raw header rather than
/// a decoded struct, because `SignatureRef::time` is a raw field here and the
/// header is what the object's id is computed over.
