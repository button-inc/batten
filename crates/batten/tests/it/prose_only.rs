...
//
// carried: mise-tasks/prose-only-check.sh policy/prose-only.rego crates/batten/tests/it/prose_only.rs
// carried: tests/prose-only-check.bats policy/prose-only.rego crates/batten/tests/it/prose_only.rs
...
// carried: "a comment-only diff with no test change is refused" crates/batten/tests/it/prose_only.rs
#[test]
...
// carried: "a comment change plus any code line is admitted" crates/batten/tests/it/prose_only.rs
#[test]
...
// carried: "a comment change plus a test change is admitted — the PR #604 shape" crates/batten/tests/it/prose_only.rs
#[test]
...
// carried: "a shell comment counts as prose, and code in the same file does not" crates/batten/tests/it/prose_only.rs
#[test]
...
// carried: "a reflowed comment block with blank lines is still prose" crates/batten/tests/it/prose_only.rs
#[test]
...
// changed: "a deleted file is not read as a comment change" crates/batten/tests/it/prose_only.rs the shell excluded every deletion wholesale because it could not classify one; the engine compares remainders, so a module deletion is still refused while a pure-prose deletion is admitted
#[test]
...
// changed: "a .md-only diff is refused — the whole file is prose" crates/batten/tests/it/prose_only.rs the shell could only reach this for an EDITED markdown file; the deletion half was excluded, and this case is the half that exclusion cost
#[test]
...
// carried: "an unrecognised extension admits the branch" crates/batten/tests/it/prose_only.rs
#[test]
...
// carried: "an empty diff is not judged" crates/batten/tests/it/prose_only.rs
#[test]
...
// carried: "no base to diff against is not judged, rather than refused" crates/batten/tests/it/prose_only.rs
#[test]
...
// changed: "the refusal names paths and a count, never a line of the diff" crates/batten/tests/it/prose_only.rs the shell printed every changed path beside the count; the port emits the count alone, because a diff is content nobody has published
#[test]
...
// `crates/batten/tests/it/admission.rs` is where every clause of it is tested —
...
//
// subsumed: "the override admits the branch and records which one it admitted" crates/batten/tests/it/admission.rs
// subsumed: "the remedy names where the content should go, not merely a flag" crates/batten/tests/it/verdict_registry.rs
