...
// `unconserved_cases`).
//
// carried: "config-deprecations.bats::a schema that lost no key exits 0" crates/batten/tests/config_deprecations.rs
// carried: "config-deprecations.bats::an unannounced removal is reported rather than passed" crates/batten/tests/config_deprecations.rs
// carried: "config-deprecations.bats::no release tag is exit 3 rather than a clean pass" crates/batten/tests/config_deprecations.rs
// carried: "config-deprecations.bats::a tag carrying no published schema is exit 3 rather than a clean pass" crates/batten/tests/config_deprecations.rs
// carried: "config-deprecations.bats::the baseline is the newest tag by version order, not by creation time" crates/batten/tests/config_deprecations.rs
// carried: "config-deprecations.bats::output is pointer-only — no schema body echoed" crates/batten/tests/config_deprecations.rs
// carried: "config-deprecations.bats::the gate leaves the tree it judges unmodified" crates/batten/tests/config_deprecations.rs
//
// changed: "config-deprecations.bats::this repo's own schema has lost no key since its last release — the gate on the real tree" crates/batten/tests/config_deprecations.rs the retired suite asserted exit 0 unconditionally over the real tree, which is only answerable in a clone that FETCHED TAGS — and a filtered or shallow clone has none, where the program itself exits 3. The case is carried as `this_repositorys_schema_has_lost_no_key_since_its_last_release`, which asserts the same verdict where a baseline resolves and asserts the could-not-look answer where none does, so it can no longer pass by having compared nothing
